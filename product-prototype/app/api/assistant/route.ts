...
- 今日焦点：3 条高意向询盘等待接管；刺梨项目询盘转化低于目标 22%；第二轮投流预算需确认。
- 项目：贵州抹茶｜东南亚市场，商机推进，目标达成 78%，商机管道 ¥486 万，投入产出 6.8x。
- 项目：工业轮胎｜中东市场，获客验证，目标达成 71%，商机管道 ¥842 万，投入产出 7.4x。
- 客户工作台已知：Adrian Tan（Lumi Ingredients，马来西亚）意向分 91；Omar Farooq（Gulf Pantry Trading，阿联酋）意向分 89，CIF Dubai 报价正等待销售总监确认折扣。
`;
...
function safeFallback(message: string, view: string) {
  ...
  if (/本月|今天|关注|异常|重点|总结/.test(message)) {
    return `结论：当前最需要关注三件事。\n\n已知事实：3 条高意向询盘等待接管；刺梨项目询盘转化低于目标 22%；第二轮投流预算需确认。\n\n建议：先处理高意向询盘，再核对刺梨项目的转化漏损，最后结合预期询盘质量审批投流预算。`;
  }
