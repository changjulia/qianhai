...
export default function Home() {
  ...
  return <main className={sidebarCollapsed ? 'app-shell sidebar-is-collapsed' : 'app-shell'}>
    ...
    <section className="workspace">
      ...
      <div className="page">{view==='home'?<HomePage go={go}/>:view==='projects'?<ProjectsPage/>:view==='agents'?<AgentsPage/>:view==='approvals'?<ApprovalsPage/>:view==='content'?<ContentPage go={go}/>:view==='schedule'?<SchedulePage/>:view==='distribution'?<DistributionPage/>:view==='traffic'?<TrafficPage/>:view==='inquiries'?<CustomerOperationsOverview onOpenWorkspace={openWorkbench} onOpenRevenueAnalysis={()=>go('revenue')}/>:view==='workbench'?<CustomerWorkbenchPage key={`${workbenchTarget.tab}-${workbenchTarget.filter}-${workbenchTarget.customerId}`} initialTab={workbenchTarget.tab} initialFilter={workbenchTarget.filter} initialCustomerId={workbenchTarget.customerId} onBack={()=>go('inquiries')}/>:view==='customerLive'?<InquiriesPage go={go}/>:view==='customers'?<CustomersPage go={go}/>:view==='revenue'?<RevenueAnalysisPage onOpenWorkbench={openCustomerInWorkbench}/>:<PlatformManagementPage key={view} view={view as PlatformView}/>}</div>
    </section>
