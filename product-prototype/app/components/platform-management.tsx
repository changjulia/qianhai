...
function DemoShell({ children }: { children: ReactNode }) {
  ...
  const complete = () => {
    if (!action) return;
    notify(`${action.title}已完成（演示数据）`);
    setAction(null);
  };
  return <DemoContext.Provider value={{ open: setAction, notify }}>
    {children}
    {action && <div className="demo-overlay" role="presentation" onMouseDown={() => setAction(null)}>
      <section className="demo-dialog" role="dialog" aria-modal="true" aria-labelledby="demo-dialog-title" onMouseDown={event => event.stopPropagation()}>
        <header><div><span>可交互 Demo</span><h2 id="demo-dialog-title">{action.title}</h2><p>{action.desc}</p></div><button aria-label="关闭" onClick={() => setAction(null)}>×</button></header>
        {action.fields && <div className="demo-form">{action.fields.map((field, index) => <label key={field}><span>{field}</span>{index === action.fields!.length - 1 && action.fields!.length > 2 ? <textarea defaultValue="演示环境中的示例配置"/> : <input defaultValue={index === 0 ? '演示方案' : ''}/>}</label>)}</div>}
        <div className="demo-dialog-note"><strong>演示模式</strong><p>操作只改变当前浏览器里的演示状态，不会写入真实账号、客户或业务系统。</p></div>
        <footer><button className="secondary" onClick={() => setAction(null)}>取消</button><button className="primary" onClick={complete}>{action.confirm ?? '确认并保存'}</button></footer>
      </section>
    </div>}
...
function PlatformManagementContent({ view }: { view: PlatformView }) {
  ...
  return <>
    <PageHeader title={title} desc={config.desc} action={config.action} secondary={config.secondary} onAction={() => open({ title: config.action ?? '新建配置', desc: `在${title}中创建一条新的演示配置。`, fields: ['名称', '所属组织', '说明'] })} onSecondary={() => open({ title: config.secondary ?? '查看详情', desc: `查看${title}的检查结果和建议。`, confirm: '标记为已查看' })}/>
...
function SectionIntro({ title, desc, action }: { title: string; desc: string; action?: string }) {
  const { open } = useDemo();
  return <div className="panel-title platform-title"><div><h2>{title}</h2><p>{desc}</p></div>{action && <button onClick={() => open({ title: action, desc: `调整“${title}”的演示规则，保存后会显示操作反馈。`, fields: ['规则名称', '适用范围', '规则说明'] })}>{action}</button>}</div>;
}
  ...
  return <section className="panel org-panel platform-panel">
    ...
    {tab === '组织视图' ? <div className="org-layout enhanced-org">
      ...
      <div className="org-detail org-detail-rich">
        <div className="detail-toolbar"><div><span>{current.type}</span><h2>{current.name}</h2><p>负责所属品牌和经营任务的目标配置、预算治理、内容增长与客户结果。</p></div><button className="secondary" onClick={() => open({ title: `编辑${current.name}`, desc: '修改组织名称、负责人和数据边界。', fields: ['组织名称', '负责人', '数据边界'] })}>编辑节点</button></div>
...
function CollaborationBoundaries() {
  ...
  return <div className="platform-table"><div className="platform-row head"><span>协作关系</span><span>共享对象</span><span>协作方式</span><span>明确边界</span><span>状态</span></div>{rows.map(row => <button className="platform-row" key={row[0]} onClick={() => open({ title: row[0], desc: `当前共享：${row[1]}；数据边界：${row[3]}。`, fields: ['共享对象', '有效期', '补充限制'] })}>{row.map((cell, index) => <span key={cell} className={index === 0 ? 'strong-cell' : index === 4 ? (cell === '正常' ? 'status-good' : 'status-warn') : ''}>{cell}</span>)}</button>)}</div>;
}
...
function DataScopeMatrix() {
  ...
  return <div className="scope-list">{domains.map((domain, index) => <article key={domain[0]}><span className={`scope-icon s${index}`}>{index + 1}</span><div><strong>{domain[0]}</strong><small>{domain[1]}</small></div><p>{domain[2]}</p><em>{domain[3]}</em><button onClick={() => open({ title: `配置${domain[0]}权限`, desc: domain[2], fields: ['适用角色', '数据范围', '字段限制'] })}>配置</button></article>)}</div>;
}
...
function AccountGrid() {
  ...
  return <div><div className="account-summary"><span>账号使用规则</span><strong>个人身份、平台账号与数据范围分开授权；数字员工只能使用绑定到经营任务的账号。</strong><button onClick={() => open({ title: '连接新账号', desc: '选择平台并完成授权范围确认。', fields: ['平台名称', '账号名称', '授权范围'], confirm: '模拟连接' })}>连接新账号</button></div><div className="account-grid enhanced-accounts">{accounts.map((account, index) => <button key={account[0]} onClick={() => open({ title: `管理${account[0]}`, desc: `${account[1]}；当前范围：${account[2]}；状态：${account[3]}。`, fields: ['账号名称', '所属范围', '到期时间'] })}><span className={`platform-icon p${index}`}>{account[4]}</span><span><strong>{account[0]}</strong><small>{account[1]}</small><i>{account[2]}</i></span><em className={account[3] === '正常' ? 'good' : 'warn'}>{account[3]}</em></button>)}</div></div>;
}
...
function ApprovalChains() {
  ...
  return <div className="approval-chain-list">{chains.map(chain => <article key={chain[0]}><span className="chain-number">审</span><div><strong>{chain[0]}</strong><small>{chain[1]}</small></div><p>{chain[2]}</p><em>{chain[3]}</em><button onClick={() => open({ title: `编辑${chain[0]}审批链`, desc: `当前流程：${chain[1]}。`, fields: ['触发条件', '审批人', '处理时限'] })}>编辑</button></article>)}</div>;
}
...
function MasterData() {
  ...
  return <div className="master-layout"><div className="master-flow"><span>源系统</span><i>→</i><span>字段映射</span><i>→</i><span>统一业务对象</span><i>→</i><span>权限与归因</span></div><div className="platform-table master-table"><div className="platform-row head"><span>统一对象</span><span>主数据来源</span><span>关键字段</span><span>更新策略</span><span>状态</span></div>{objects.map(object => <button className="platform-row" key={object[0]} onClick={() => open({ title: `查看${object[0]}映射`, desc: `主数据来源：${object[1]}；更新策略：${object[3]}。`, fields: ['来源字段', '统一字段', '转换规则'] })}>{object.map((cell, index) => <span key={cell} className={index === 0 ? 'strong-cell' : ''}>{cell}</span>)}<span className="status-good">已定义</span></button>)}</div></div>;
}

function DataQuality() {
  const { open } = useDemo();
  return <div className="data-quality-layout"><div className="health-score large"><b>96.8%</b><span>整体完整率</span><small>较上周 +1.2%</small></div><div className="quality-cards">{[['重复客户记录', '27', '建议合并', '客户主数据'], ['待匹配询盘', '14', '需要确认', '客服渠道'], ['异常渠道数据', '2', '正在重试', 'Meta'], ['缺少同意状态', '8', '禁止自动外发', '联系人']].map(item => <button key={item[0]} onClick={() => open({ title: `处理${item[0]}`, desc: `${item[3]}发现 ${item[1]} 条问题，系统建议：${item[2]}。`, fields: ['处理方式', '负责人', '备注'], confirm: '模拟处理' })}><span>{item[0]}</span><b>{item[1]}</b><small>{item[2]}</small><em>{item[3]}</em></button>)}</div></div>;
}
...
function DataAccessLog() {
  ...
  return <div className="platform-table access-table"><div className="platform-row head"><span>时间</span><span>访问主体</span><span>数据动作</span><span>权限依据</span><span>结果</span></div>{logs.map(log => <button className="platform-row" key={`${log[0]}-${log[1]}`} onClick={() => open({ title: `${log[4]}：${log[2]}`, desc: `访问主体：${log[1]}；权限依据：${log[3]}；发生时间：${log[0]}。`, confirm: '关闭详情' })}>{log.map((cell, index) => <span key={cell} className={index === 4 ? (cell === '允许' ? 'status-good' : 'status-danger') : ''}>{cell}</span>)}</button>)}</div>;
}
...
function AISecurity() {
  ...
  return <div className="ai-policy-list">{policies.map((policy, index) => <article key={policy[0]}><span className="policy-index">0{index + 1}</span><div><strong>{policy[0]}</strong><p>{policy[1]}</p></div><em>{policy[2]}</em><button onClick={() => open({ title: policy[0], desc: policy[1], fields: ['适用范围', '执行方式', '例外审批'] })}>查看策略</button></article>)}</div>;
}
...
function AuditAndAlerts() {
  ...
  return <div className="audit-layout"><div className="audit-summary"><div><b>248</b><span>今日审计事件</span></div><div><b>3</b><span>自动拦截</span></div><div><b>7</b><span>等待审批</span></div><div><b>0</b><span>未处置高危</span></div></div><div className="platform-table audit-table"><div className="platform-row head"><span>事件</span><span>说明</span><span>策略域</span><span>级别</span><span>时间</span></div>{events.map(event => <button className="platform-row" key={event[0]} onClick={() => open({ title: event[0], desc: `${event[1]}。策略域：${event[2]}，风险级别：${event[3]}，时间：${event[4]}。`, fields: ['处置状态', '负责人', '处理说明'], confirm: '模拟处置' })}>{event.map((cell, index) => <span key={cell} className={index === 0 ? 'strong-cell' : index === 3 ? `risk-${cell === '高' ? 'high' : cell === '中' ? 'medium' : 'low'}` : ''}>{cell}</span>)}</button>)}</div></div>;
}
