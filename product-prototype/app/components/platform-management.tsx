...
function Accounts() {
  ...
  return <div><div className="account-summary"><span>演示连接</span><strong>Demo 数据可直接浏览；接入真实平台时再配置服务端凭证。</strong></div><div className="account-grid enhanced-accounts">{(data?.integrations ?? []).map((item,index) => <button key={str(item.id)} aria-label={`配置连接 ${item.name}`} onClick={() => openIntegration(open,item)}><span className={`platform-icon p${index%6}`}>{str(item.name).slice(0,1)}</span><span><strong>{str(item.name)}</strong><small>{label(item.integration_type)}</small><i>查看配置</i></span><em className={item.status === 'connected' || item.status === 'demo_ready' ? 'good' : 'warn'}>{label(item.status)}</em></button>)}</div></div>;
}
...
function Integrations() { const { data,open,mutate,notify } = usePlatform(); return <><div className="integration-principle"><span>演示数据已就绪</span><strong>Demo 连接展示完整业务数据；真实连接仍需服务端凭证。</strong><small>页面不会把模拟连接冒充为生产连接</small></div><div className="integration-table"><div className="integration-row head"><span>系统</span><span>范围</span><span>方式</span><span>方向</span><span>状态</span><span>操作</span></div>{(data?.integrations ?? []).map((item) => <div className="integration-row" key={str(item.id)}><span><i className="system-cube">系</i><strong>{str(item.name)}</strong></span><span>{strings(item.scopes_json).join('、') || '待定义'}</span><span>{label(item.auth_method,'演示数据')}</span><span>{label(item.sync_direction,'只读')}</span><span className={item.status === 'connected' || item.status === 'demo_ready' ? 'status-good' : 'status-warn'}>{label(item.status)}</span><span><button onClick={() => openIntegration(open,item)}>查看</button>{item.status !== 'demo_ready' && <button onClick={() => void mutate('sync.run',{integrationId:item.id}).then((r) => notify(r.message || '同步已提交')).catch((e: Error) => notify(e.message))}>同步</button>}</span></div>)}</div></>; }
...
function Alerts() { const { data,open } = usePlatform(); const events = data?.auditEvents ?? []; const actionable=events.filter((e) => e.result !== 'success'); const highOpen=events.filter((e) => e.risk_level === 'high' && e.result !== 'success' && !e.disposition); return <div className="audit-layout"><div className="audit-summary"><div><b>{events.length}</b><span>最近事件</span></div><div><b>{actionable.length}</b><span>需要关注</span></div><div><b>{events.filter((e) => e.risk_level === 'high' && e.result !== 'success').length}</b><span>高风险异常</span></div><div><b>{highOpen.length}</b><span>未处置高危</span></div></div><div className="platform-table audit-table"><div className="platform-row head"><span>事件</span><span>主体</span><span>资源</span><span>级别</span><span>时间</span></div>{events.map((e) => <button className="platform-row" key={str(e.id)} onClick={() => open({title:str(e.action),desc:`结果：${label(e.result)}`,kind:e.result !== 'success'?'audit':'detail',operation:e.result !== 'success'?'audit.resolve':undefined,values:{eventId:e.id},context:[str(e.action),str(e.result),str(e.occurred_at)]})}><span className="strong-cell">{str(e.action)}</span><span>{str(e.actor_id,str(e.actor_type))}</span><span>{str(e.resource_type)}</span><span className={`risk-${e.risk_level}`}>{label(e.risk_level)}</span><span>{str(e.occurred_at).replace('T',' ').slice(0,16)}</span></button>)}</div></div>; }
